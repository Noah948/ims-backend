...
def delete_category_field(
    ...
    if not category or not category.fields:
        return False
    new_fields = [field for field in category.fields if field["id"] != field_id]
...
def reorder_category_fields(
    ...
    if not category or not category.fields:
        return None
    existing_fields = {field["id"]: field for field in category.fields}
    ...
    for index, field_id in enumerate(ordered_field_ids, start=1):
        ...
        new_fields.append({**field, "order": index})
    category.fields = new_fields
    db.commit()
    db.refresh(category)
...
def delete_category(db: Session, category_id: str, business_id: str):
    category = db.query(Category).filter(
        Category.id == category_id,
        Category.business_id == business_id,
        Category.deleted_at.is_(None)
...
def cleanup_deleted_categories(db: Session):
    ...
    (
        db.query(Category)
        .filter(Category.deleted_at.is_not(None), Category.deleted_at < cutoff)
